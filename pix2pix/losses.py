...
def CustomLoss(
    input_, output, target, mask, B_ch0_scalefactor, mask_type, nonzero_L1weight, rms, name=''
):
    """
    """
    if mask_type == 'auto':
        ...
        input_chs = input_[0, 0, :, :]
        output_chs = output[0, 0, :, :]
        target_chs = target[0, 0, :, :]

        right_roll5 = target_chs.roll(5, 1)
        right_roll5[:, :5] = 5 * B_ch0_scalefactor # I'm not sure what these lines do
        ...
        loss_pix = ( # L1 pixelwise
            ((peak_mask * output_chs) - (peak_mask * target_chs)).abs().sum() / 
            peak_mask.sum()
        )
        # I dont think this works for induction views
        loss_channel = ( # L1 on sum of non-zero channels in image
            ((peak_mask * target_chs).sum(1) - (peak_mask * output_chs).sum(1)).abs().sum() / 
            peak_mask.sum(1).count_nonzero()
        )
        ...
        # NOTE should make induction an opt flag
        ...
        if induction:
            # L1 on negative and positive sum of non-zero channels in image
            loss_channel_positive = (
                (
                    (mask * target * (target >= 0)).sum(3) - (mask * output * (target >= 0)).sum(3)
                ).abs().sum() / 
                mask.sum(3).count_nonzero()
            )
            loss_channel_negative = (
                (
                    (mask * target * (target < 0)).sum(3) - (mask * output * (target < 0)).sum(3)
                ).abs().sum() /
                mask.sum(3).count_nonzero()
            ...
            # L1 on sum of non-zero channels in image
            loss_channel = (
                ((mask * target).sum(3) - (mask * output).sum(3)).abs().sum() / 
                mask.sum(3).count_nonzero()
            )
